...
my $_END = sub {
		...
		#       or fail "can not copy ".$globals->{'result_dir'}."/../test_status to".$globals->{'result_dir'}."/.";
                ...
                if(defined $globals->{'testkitdone'} && $globals->{'testkitdone'} eq "Finished"){
                       $status_summary = "Finished";
                }
                $status_summary = "Auto: $status_summary\n";
                write_string_as_file($globals->{'result_dir'}."/test_status", $status_summary);
...
sub get_options {
        ...
        push @$params, {LONG_NAME=>'userprofile',        SHORT_NAME=>'u',       HAS_VALUE=>1};
        push @$params, {LONG_NAME=>'report',             HAS_VALUE=>1};
        push @$params, {LONG_NAME=>'testrun_id',         SHORT_NAME=>'t',       HAS_VALUE=>1};
...
sub Setup {
        ...
        if($globals->{'not-run'} && $options->{'report'}) {
                ...
                if($options->{'testrun_id'}) {
                        rebuild_report($options->{'report'},$RESULTS_DIR."/".$options->{'testrun_id'}, $options->{'profile'});
                }else{
        ...
        if($globals->{'not-run'} && $options->{'mail-to'}){
                ...
                if($options->{'testrun_id'}) 
                {
			$globals->{'results_tarball'} = $RESULTS_DIR."/".$options->{'testrun_id'}.".tgz";

			my $tmp_file = $globals->{'temp_dir'}."/".extract_filename($globals->{'results_tarball'});
                        
			cmd( "cd ".extract_dir($RESULTS_DIR."/".$options->{'testrun_id'})
				." && tar czf ".$tmp_file." ".extract_filename($RESULTS_DIR."/".$options->{'testrun_id'})
				." && mv ".$tmp_file." ".$globals->{'results_tarball'}
				);
        ...
        $globals->{'userprofile'} = $FindBin::Bin."/../../profiles/user/user.profile";
        ...
        if ( $options->{'profile'} ) {
                ...
                write_string_as_file($globals->{'result_dir'}."/".$globals->{'auto_profile'}, $profile_auto);
       ...
       if (-f $globals->{'userprofile'}) {
                cmd( "cp -f ".$globals->{'userprofile'}." ".$globals->{'result_dir'}."/profile.user" ) == 0
                        or warning "Failed to copy profile: '".$globals->{'user_profile'}."' to '".$globals->{'result_dir'}."'";
       } 
       ...
       if(-f $globals->{'userprofile'}){
        ...
        inform "==========================================";
        inform $MTK_BRANCH." MeeGo Testkit v. ".$MTK_VERSION;
...
sub write_status {
		...
		my $status = $globals->{'testkitdone'} ? $globals->{'testkitdone'} : 'Prepare';;
		...
		$content .= "PREPARE_PERCENT = "." #Reserved\n";
                my $progress = ".";
                $progress = $globals->{'testkit_lite_progress'}, if defined $globals->{'testkit_lite_progress'}; 
...
sub Check_subshell_off {
        ...
        if ( $self->{SUBSHELL} && !$self->{SUBSHELL}{TERMINATED} ) {
                my $pid_info = ( $self->{SUBSHELL}{PTY_PID} ? "PTYSHELL: ".$self->{SUBSHELL}{PTY_PID} : "")
                                .( $self->{SUBSHELL}{SUB_PID} ? ", SUBSHELL: ".$self->{SUBSHELL}{SUB_PID} : "");

                inform "Terminating ".$self->name." (".$pid_info.")";
                if ( !is_ok $self->{SUBSHELL}->ExpectLogout(0) ) {
                        $self->report_error( complain "Attention! Failed to shutdown a subshell."
                                        ."\nPlease, check the process list and kill the following processes: ".$pid_info,
                                $Error::Last );
                } else {
...
$SIG{INT} = sub {
        ...
        return if(defined $globals->{'done'} && $globals->{'done'} eq "" && $globals->{'done'} eq "Finished") ;

        # exit subshell
        if(defined $globals->{'subshell'}){
		my $pid_info = ( $globals->{'subshell'}{PTY_PID} ? "PTYSHELL: ".$globals->{'subshell'}{PTY_PID} : "")
					.( $globals->{'subshell'}{SUB_PID} ? ", SUBSHELL: ".$globals->{'subshell'}{SUB_PID} : "");
...
sub Spawn_subshell {
        ...
        is_ok $subshell->Spawn($user) # TODO: compare all tests with $user=undef vs. $user='root'
        ...
        is_ok $subshell->Send("export LC_ALL=C; export LC_MESSAGES=C; export LC_COLLATE=C; export LC_CTYPE=C; export LC_MONETARY=C; export LC_NUMERIC=C; export LC_TIME=C; \n") # To avoid messages in local languages
                or return error "Failed to send command to the started subshell", $Error::Last;
...
# test prepration
{
	...
	cmd("cp -f ".$globals->{'result_dir'}."/".$globals->{'auto_profile'}." ".$globals->{'testkit_lite_dir'}."/testkit") == 0
	   or fail "Can not copy ".$globals->{'result_dir'}."/".$globals->{'auto_profile'}." to ".$globals->{'testkit_lite_dir'}."/testkit";
	...
	if(-d $globals->{'testlog_dir'}."/runtest/latest"){
	   cmd("rm -rf ".$globals->{'testkit_lite_dir'}."/log/runtest/latest") == 0
	   or warning "Can not clean up latest log under ".$globals->{'testkit_lite_dir'}."/log/runtest/latest";
	}
}
...
sub getBackupResultXMLCMD{
	...
	if($profile_path && $result_path){
		...
		foreach (@thisTargetPackages) {
			...
			push (@copyCommands, "cp -rf /opt/testkit/lite/latest/usr/share/$_/tests.xml.xmlresult $result_path/$_/result.tests.xml");
			push (@copyCommands, "more /opt/testkit/lite/latest/usr/share/$_/tests.xml.textresult >> $result_path/auto_summary");
		}
	}
...
# run test in ptyshell
{
	...
	my $profile_content = &readProfile($globals->{'testkit_dir'}."/tests/testkit"); 
    inform "[CMD]:\nrm -rf ".$globals->{'testlog_dir'}."/runtest/latest/*;\n testkit-lite -f $profile_content;\nexit\n";
	...
	$subshell->Send("rm -rf ".$globals->{'testlog_dir'}."/runtest/latest/*;testkit-lite -f $profile_content;exit\n"); 
	...
	if ( $subshell->{EXIT_CODE} ) { 
	     warning "subshell exited with code ".$subshell->{EXIT_CODE} 
...
# backup report and send email
{
	...
	if(-f $globals->{'testlog_dir'}."/runtest/latest/tests.summary"){
		inform "[CMD]:cp -f ".$globals->{'testlog_dir'}."/runtest/latest/tests.summary ".$globals->{'result_dir'}."\/autotest.res";
	     cmd("cp -f ".$globals->{'testlog_dir'}."/runtest/latest/tests.summary ".$globals->{'result_dir'}."\/autotest.res");
	}else{
	...
	my $backXmlCmds = &getBackupResultXMLCMD($globals->{'testkit_dir'}."/tests/testkit", $globals->{'result_dir'});
	foreach (@$backXmlCmds){
		cmd($_);
	}
	...
	if( $globals->{'result_dir'} ) {
	     rebuild_report($globals->{'report'},$globals->{'result_dir'}, $globals->{'original_profile_path'});
		...
		cmd( "cd ".shq(extract_dir($globals->{'result_dir'}))
			." && tar czf ".shq($tmp_file)." ".shq( extract_filename($globals->{'result_dir'}) )
