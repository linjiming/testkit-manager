...
sub push_result_back(){
	...
	if($this_result_path){
		...
		foreach (@$pakRef){
			my $push_cmd = "cp -rf $this_result_path/$_/result.tests.xml /opt/testkit/lite/latest/usr/share/$_/tests.xml.xmlresult";
			$TestKitLogger::logger->log(message =>  "Push the Result File back, CMD: $push_cmd");
			system($push_cmd);
		}
		no ProcessSummary;
	}
...
sub send_reply() {
	if ($error_text) {
		...
		$error_text =~ s/]]>/]]>]]&gt;<![CDATA[/g;              # Escape ]]> that would close CDATA block otherwise
		print $output_xml."<root>\n<error><![CDATA[$error_text]]></error>\n</root>";
	} elsif($isJson){
		my $response = "HTTP/1.0 200 OK" . CRLF . "Content-type: application/json" . CRLF . CRLF . "$data";
		$TestKitLogger::logger->log(message =>  "[ajax_srv.pl]: the response:\n$response");
		print $response;
	} elsif($isWholeXML){
...
sub list_subdir($) {
        ...
        if (opendir(DIR, $full_path)) {
                ...
                $error_text = "Could not obtain directory list! $full_path<br />($!)";
        }
        }
        ...
        if (($curr_dir eq '') || ($profile eq '')){
                ...
                if (open(PROFILE, $profile)) {
					...
					if ($line =~ /(.*)\s+(.*)\s+(PASS|FAIL|BLOCK|NotRun)/) {
                                        	($suitname, $casename, $teststatus) = split(/\s+/, $line);
					}
                                        $data .= "<testcase> \n";
                                ...
                                $data .="<testcases>no profiled cases in this testsuit $profile</testcases>\n";
                        }
                }
                else {
                        $error_text = "Could not obtain profiled cases list! $profile<br />($!)";
                }
        }
}
...
sub save_case_result($$) {
        ...
        my $result_file_new = "/root/testkit/testkit-git/moblin-testkit-bak/manager/webui/public_html/manual.res";
        ...
        for (my $i = 0; $i < $scalar; $i++) {
                foreach my $result_item (@$new_result) {
                        if (($full_result[$i]->{'suitname'} eq $result_item->{'suitname'}) &&
                            ($full_result[$i]->{'casename'} eq $result_item->{'casename'})) {
...
sub load_profile($) {
    ...
    while ( <FILE> ) {
		my $line = $_;
		$TestKitLogger::logger->log(message =>  "[ajax_srv.pl]: Profile line:$line");
		if ($line !~ s/(\[Auto\]|\[Manual\])// ){
			$line =~ s/\n//g;
			$TestKitLogger::logger->log(message =>  "[ajax_srv.pl]: Processed test_package:$line");
...
sub construct_progress_data($) {
	...
	if (!$status->{'TEST_SUITES'} || scalar(@{$status->{'TEST_SUITES'}}) == 0) {
	...
	foreach my $ts_info (@{$status->{'TEST_SUITES'}}) {
		...
		elsif (($ts_info->{'STATUS'} eq 'Running') || ($ts_info->{'STATUS'} eq 'Making report')) {
			if ($ts_info->{'PREPARE_PERCENT'}) {
				$prepare_percent = $ts_info->{'PREPARE_PERCENT'};
			}
		...
		elsif (($ts_info->{'STATUS'} eq 'Failed') || ($ts_info->{'STATUS'} eq 'Warnings') || ($ts_info->{'STATUS'} eq 'Passed') || ($ts_info->{'STATUS'} eq 'No verdict') || ($ts_info->{'STATUS'} eq 'Incomplete')) {
			if ($ts_info->{'PREPARE_PERCENT'}) {
				$prepare_percent = $ts_info->{'PREPARE_PERCENT'};
			}
		...
		$res .= "<testsuite id=\"$id\"><name>$ts_name</name><status>$ts_status</status><elapsed>$elapsed_time</elapsed><total>$total_time</total><prep_percent>$prepare_percent</prep_percent><percent>$current_percent</percent></testsuite>";
	}
	$res .= '</progress>';
	return $res;
}
...
if ($SERVER_PARAM{'PROXY'}) {
	$proxy_option .= ' --proxy='.$SERVER_PARAM{'PROXY'}.','.$SERVER_PARAM{'PROXY_AUTH'};
}
else {
	if ($SERVER_PARAM{'HTTP_PROXY'}) {
		$proxy_option .= ' --http-proxy='.$SERVER_PARAM{'HTTP_PROXY'}.','.$SERVER_PARAM{'HTTP_PROXY_AUTH'};
	}
	if ($SERVER_PARAM{'FTP_PROXY'}) {
		$proxy_option .= ' --ftp-proxy='.$SERVER_PARAM{'FTP_PROXY'}.','.$SERVER_PARAM{'FTP_PROXY_AUTH'};
	}
}
...
if (!$_GET{'action'}) {
	...
	if (!-d $SERVER_PARAM{'APP_DATA'}.'/profiles/test') {
		...
		if (!-d $SERVER_PARAM{'APP_DATA'}.'/profiles/test') {
			$error_text = 'Could not create profile directory:<br />'.$SERVER_PARAM{'APP_DATA'}.'/profiles/test';
		}
	}
	if (!$error_text) {
		if ($_COOKIE{'session_id'}) {
			$full_save_name = $SERVER_PARAM{'APP_DATA'}.'/profiles/test/~session.'.$SERVER_PARAM{'PEER_IP'}.'.'.$_COOKIE{'session_id'};
		}
		else {
			$error_text = 'Cannot retrieve session ID! Please, allow cookies in your browser.';
		}
	}
	if (!$error_text) {
	...
	if (!$error_text) {
		...
		if (defined($tests_status) && $tests_status->{'IS_RUNNING'}) {
			$error_text = 'Tests are already running!<br />You cannot run another instance before they are finished.<br />You can watch or stop the running tests on the <a href="tests_exec.pl"><u>Execution</u></a> page.';
		}
	}
	if (!$error_text) {
		if (open(CHK_FILE, $CONFIG{'TESTS_DIR'}."/dist-checker.pl --webui --check-only -f \"$full_save_name\" 3>&1 1>/dev/null 2>&3 |")) { # capture STDERR only
			my $chk_text = '';
			while(<CHK_FILE>) {
				$chk_text .= $_;
			}
			...
			if ($chk_text =~ m/^Error:/mi) {
				...
				$error_text =~ s/\r?\n/<br \/>/sg;
				$error_text .= '<br />';
				$error_text .= '<br />Please, see the <a href="tests_help.pl#troubleshooting"><u>Troubleshooting</u></a> Help section to find the solution.';
				$error_text .= '<br />If you could not find any, please, write at <a href="mailto:linux@ispras.ru"><u>linux@ispras.ru</u></a>.';
			}
		}
		else {
			$error_text = "Cannot run dist-checker.pl to check the requirements!<br />$!";
		}
	}
...
elsif ($_GET{'action'} eq 'run_tests') {                    # Start the tests
        if (!$_GET{'profile'}) {
                $error_text = 'Incorrect call parameters!';
        }
        else {
                ...
                if ($status->{'IS_RUNNING'}) {
                        $error_text = "Tests are already running.<br />You cannot run another instance before they are finished.<br />Start watching the current run?<br /><br /><input type=\"button\" value=\"Yes\" style=\"width: 5em;\" onclick=\"javascript:cert='".($status->{'CERTIFICATION'} ? 'certification' : 'custom')."';startTestsPrepareGUI();startRefresh();\" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"button\" value=\"No\" style=\"width: 5em;\" onclick=\"javascript:err_area.style.display='none';\" />";
                }
                else {
                        my $profile_path = $SERVER_PARAM{'APP_DATA'}.'/profiles/test/'.$_GET{'profile'};
                        #my $tef_stderr = tmpnam();
                        ...
                        if (!defined($child)) {
								...
								exec($CONFIG{'TESTS_DIR'}."/testkit_lite_start.pl -f '$profile_path'  -w  2>$tef_stderr");
                        }
                        else{
								# Parent process
                                ...
                                while (1) {
										...
										$TestKitLogger::logger->log(message =>  "[ajax_srv.pl]: The profile = $profile_path tef_stderr=$tef_stderr is_running= $is_running, reply = $tef_reply\n");
                                        ...
                                        if (!is_ok($tef_reply)) {
                                                ...
                                                if (!$status->{'IS_RUNNING'}) {
                                                        ...
                                                        if ($is_running) {
                                                                $error_text = "Internal error: .pl started but claims it is not running. status=".$status->{'IS_RUNNING'};
                                                        }
                                                        else {
                                                                # .pl started and finished immediately.
                                                                $data .= construct_progress_data($status);
																$TestKitLogger::logger->log(message =>  "[ajax_srv.pl]: started and finished data = $data\n");
                                                                ...
                                                                if ($status->{'STATUS'} eq 'Finished') {
                                                                        ...
                                                                        $data .= "<redirect>$tid</redirect>\n";
                                                                }
                                                                elsif ($status->{'STATUS'} eq 'Terminated') {
                                                                        # Test run was forcibly stopped; show status, do not redirect.
                                                                        $data .= "<tr_status>&lt;span style=\"color: red;\"&gt;The test has been stopped.&lt;/span&gt;</tr_status>\n";
                                                                }
                                                                else {
                                                                        # Test run has crashed; show status and do not redirect.
                                                                        $data .= "<tr_status>&lt;span style=\"color: red;\"&gt;The test seems to have crashed.&lt;/span&gt;</tr_status>\n";
                                                                }
                                                           }
                                                }
                                                else {
                                                        ...
                                                        $data .= construct_progress_data($status);
														$TestKitLogger::logger->log(message =>  "[ajax_srv.pl]: Started data = $data\n");
                                                }
                                                last;
                                       }
                                                ...
                                                $error_text = 'Internal error: .pl terminated unexpectedly.';
                                                last;
                                        }
                                        elsif (!$is_running) {
                                                ...
                                                $tef_reply =~ s/>/&gt;/g;
                                                $tef_reply =~ s/\r?\n/<br \/>/g;
                                                $error_text = 'Internal error: .pl returned unexpected output:<br />'.$tef_reply;
                                                last;
                                        }
                               }
...
elsif ($_GET{'action'} eq 'get_test_log') {                 # Get next portion of test log output
                ...
                if (lock_status()) {
                        ...
                        if (-f $log_file) {
                                ...
                                if (open(FILE, $log_file)) {
                                        ...
                                        $error_text = "Could not open output log &lsquo;$log_file&rsquo;!<br />$!";
                                }
                        }
                        else {
	...
	if (!$_GET{'tree'}) {
		...
		if ($_GET{'tree'} eq 'tests') {
			if (lock_status()) {
				...
				if ($status->{'IS_RUNNING'}) {
					if (kill('TERM', $status->{'PID'})) {
						...
						$error_text = "Failed to send SIGTERM to the process:<br />$!";
					}
				}
				else {
			...
			system("$CONFIG{'TESTS_DIR'}/stop_server.pl $SERVER_PARAM{'SERVER_PID'} >$stop_server_output 2>&1");
			if ($? == 0) {
				$data .= '<stopped id="'.$_GET{'tree'}.'">1</stopped>';
			}
			else {
...
elsif ($_GET{'action'} eq 'server_control') {               # Updates the config file and forces the server to re-read it
	my %server_params = (
		'ProxyServer' => '',
		...
		if ($_POST{$prefix.'_proxy_host'}) {
			...
			if ($_POST{$prefix.'_proxy_user'}) {
				if ($_POST{$prefix.'_proxy_pswd'}) {
					$server_params{$key} = $_POST{$prefix.'_proxy_user'}.':'.$_POST{$prefix.'_proxy_pswd'}.'@'.$server_params{$key};
				}
				else {
					$server_params{$key} = $_POST{$prefix.'_proxy_user'}.'@'.$server_params{$key};
				}
			}
			elsif ($_POST{$prefix.'_proxy_pswd'}) {
				$server_params{$key} = 'anonymous:'.$_POST{$prefix.'_proxy_pswd'}.'@'.$server_params{$key};
			}
		}
	...
	my $updating_file = 0;
	if (-f $SERVER_PARAM{'CONF_FILE'} and open(CONF_FILE, $SERVER_PARAM{'CONF_FILE'})) {
		# Read the old file and replace the parameters
		flock(CONF_FILE, LOCK_EX);
		$updating_file = 1;
	}
	elsif (-f $SERVER_PARAM{'CONF_FILE'}.'.default' and open(CONF_FILE, $SERVER_PARAM{'CONF_FILE'}.'.default')) {
	...
	my %server_params_saved = ();   # To store information which parameters have been saved
	if ($updating_file) {
	...
	close($fh_new);
	if (-f $SERVER_PARAM{'CONF_FILE'} and !unlink($SERVER_PARAM{'CONF_FILE'})) {
		$error_text = "Failed to unlink the old file $SERVER_PARAM{'CONF_FILE'}:<br />$!";
	}
	else {
		if (!rename($conf_file_new, $SERVER_PARAM{'CONF_FILE'})) {
			$error_text = "Failed to rename the new file $conf_file_new to $SERVER_PARAM{'CONF_FILE'}:<br />$!";
		}
	}
	if ($updating_file) {
	...
	if (!$_GET{'version'} or ($_GET{'version'} !~ m/^(\d+)\.(\d+)\.(\d+)-(\d+)$/)) {
		$error_text = 'Incorrect call parameters!';
	}
	else {
		my @cur_ver = ($1, $2, $3, $4);                     # Current version details
		system("wget -O /tmp/dc-update.$$ http://ftp.linuxfoundation.org/pub/lsb/test_suites/released-3.2.0/binary/runtime/");
		if ($?) {
			$error_text = 'Failed to download information!<br />Please, check your Internet connection.';
		}
		else {
			...
			if ($file =~ m/href="$pkg_name-(.+)\.([^.]+)\.rpm"/) {
				...
				if ($ver =~ m/^(\d+)\.(\d+)\.(\d+)-(\d+)$/) {
					...
					$data .= "<update>$update</update><curver>".$_GET{'version'}."</curver><updver>$ver</updver>";
				}
				else {
	...
	if ($_POST{'result'} && $_GET{'details'} && $_POST{'testcase'}) {
		my $fips_file = $CONFIG{'RESULTS_DIR'}.'/'.$_GET{'details'}.'/results/fips';
		my $fips_data = {};
		...
		if (open(my $fh, '+>>'.$fips_file)) {
			...
			foreach my $testcase (sort keys %$fips_data) {
				...
				if (defined $fips_data->{$testcase}{'RESULT'}) {
					print $fh 'RESULT: '.$fips_data->{$testcase}{'RESULT'}."\n";
				}
		...
		# Save a JS file
		my $js_file = $CONFIG{'RESULTS_DIR'}.'/'.$_GET{'details'}.'/fips.js';
	...
	if ($testrun_id && -d $CONFIG{'RESULTS_DIR'}.'/'.$testrun_id) {
		my $tarball_file = $CONFIG{'RESULTS_DIR'}.'/'.$testrun_id.'/'.$testrun_id.'.tgz';
		if (-f $tarball_file) {
			system('rm -f '.shq($tarball_file)); # Remove the old tarball
		}
		
		my $tmp_file = extract_filename($tarball_file);
		(system('cd '.shq($CONFIG{'RESULTS_DIR'})
			.' && tar czf '.shq($tmp_file).' '.shq($testrun_id)
			.' && mv '.shq($tmp_file).' '.shq($tarball_file)
			) == 0) or $error_text = 'Failed to repack the results.';
	...
	list_subdir($tests_node);
        ...
        my $tests_profile = $CONFIG{'RESULTS_DIR'}.'/'.$testrun_id.'/profile.manual';
        ...
        my $testrun_id = $_GET{'test_run'};
        my $tests_profile = $CONFIG{'RESULTS_DIR'}.'/'.$testrun_id.'/profile.manual';
	my $tests_result_dir = $CONFIG{'RESULTS_DIR'}.'/'.$testrun_id.'/results';
        my $tests_result = $tests_result_dir.'/manualtest.res';
	my @initial_result = ();
	if(! -s $tests_result_dir) {
		mkdir($tests_result_dir,0755)|| die "could not create directory $tests_result_dir";
	}
	if(! -e $tests_profile){
		my $copycmd ="cp /tmp/moblin-testkit/profiles/test/~manual_test.profile $tests_profile";
		my $ret = `$copycmd`;
	}
#	if (! -e $tests_result)
#	{
...
#	}
        if(! -e $tests_result) {
		open (NEW_FILE, ">$tests_result") || die "could not create testresult file $tests_result";
		if ((open(PROFILE, $tests_profile))){
			while (<PROFILE>)
			{
				...
				$test_suite =~ s/\r//g;
				$test_suite =~ s/\n//g;
				my $full_path = $SERVER_PARAM{'APP_DATA'}.'/tests/'.$test_suite."/testspec";
...
#							$data .= "<man_result>Inner test case: ".$tests."</man_result>";
	...
	close(NEW_FILE);
		...
		use ProcessSummary;
		my $na_num = &generateManualSummary($CONFIG{'RESULTS_DIR'}.'/'.$testrun_id);
		no ProcessSummary;
		
        my $tests_status = $CONFIG{'RESULTS_DIR'}.'/'.$testrun_id.'/test_status';
		...
		if ($$na_num == 0) {
			...
			$data .= "<man_result>$notrun_case manual case is NotRun</man_result>";	
		}
}
	...
	save_profile($profile_path, $profile_js);
        ...
        my $profile = $SERVER_PARAM{'APP_DATA'}.'/profiles/test/"'.$_GET{'profile'}.'"';
        my $rmcmd ="rm -f $profile";
        my $ret = `$rmcmd`;

        $data .= "<profile_result>Profile ".$_GET{'profile'}." remove Succeed</profile_result>";
#       $data .= "<profile_result>User ".$rmcmd." delete Succeed</profile_result>";
	...
	$data .= "<profile_result>User Load Succeed</profile_result>";
        ...
        $data .= "<profile_result>User ".$_GET{'user'}." delete Succeed</profile_result>";
#	$data .= "<profile_result>User ".$rmcmd." delete Succeed</profile_result>";
}
